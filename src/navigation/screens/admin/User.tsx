// AdminHome.tsx (with system API integration)
...
// Helper function to create file object for FormData
...
const AddProductModal: React.FC<{
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Create a single pesticide object with all data
      const pesticideData = {
      ...
      // Send all pesticide data as JSON string in a single field named 'pesticide'
      formData.append("pesticide", JSON.stringify(pesticideData));

      // Append image file
      if (image) {
        const fileObject = createFileObject(image);
        formData.append("image", fileObject as any);
      }

      console.log("Sending pesticide data as JSON:", pesticideData);
...
const StatCard: React.FC<{
  ...
  change?: string;
  loading?: boolean;
}> = ({ title, value, icon, gradientColors, change, loading }) => (
  <LinearGradient
      ...
      >
        ...
        <View style={styles.statsSection}>
          ...
          <View style={styles.statsGrid}>
            ...
            <StatCard
              title="Total Ads"
              value={stats?.advertisementCounts || 0}
              icon="image-multiple"
        ...
        <View style={styles.activitySection}>
          ...
          <View style={styles.activityList}>
            <ActivityItem
              icon="account-plus"
              title={`New farmer registered • Total: ${stats?.farmerCounts || 0}`}
              time="Just now"
              color="#6366F1"
            />
            <ActivityItem
              icon="store-plus"
              title={`New merchant joined • Total: ${stats?.merchantCounts || 0}`}
              time="5 minutes ago"
              color="#F59E0B"
            />
            <ActivityItem
              icon="package-variant"
              title={`Pesticides in inventory • Total: ${stats?.pesticideCounts || 0}`}
              time="1 hour ago"
              color="#10B981"
            />
            <ActivityItem
              icon="image-multiple"
              title={`Active advertisements • Total: ${stats?.advertisementCounts || 0}`}
              time="2 hours ago"
              color="#EF4444"
            />
          </View>
